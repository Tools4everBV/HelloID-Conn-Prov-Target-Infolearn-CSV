    ...
    try {
        ...
        foreach ($contract in $p.contracts) {
            if ($contract.Context.InConditions -OR ($dryRun -eq $True)) {
                ...
                # 20230407KM: 20221013-031 Extensions are now available from the source
                $csvRowObject."BIGNummer" = "$($contract.Custom.BIGNummer)"
                $csvRowObject."InschrijvingKwaliteitsrg" = "$($contract.Custom.InschrijvingKwaliteitsrg)"
                $csvRowObject."AanvulProfielcode" = "$($contract.Custom.AanvulProfielcode)"
